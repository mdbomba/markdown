...
curl -sk -X POST "https://<host>:<port>/accessv2" \
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"showlocalreservedports"}'
```

### APIv1 (Fallback)

```text
GET https://<host>:<port>/access/showlocalreservedports
```
...
```bash
curl -sk -u "bal:PASSWORD" "https://10.0.0.14/access/showlocalreservedports"
```
