...
- Successfully retrieved license types with valid credentials on firmware 7.2.63.x
- Response is JSON embedded within the XML `<Success>` element (not pure XML)
- Response structure: `{"categories":[{"licenseTypes":[...],"description":"...","purchaseOptions":[...]}]}`
- Two categories returned for a VLM: "Temp License" (30-day trial) and "Free License"
- Each license type includes an `id` field (SHA1 hash) needed for `alsilicense`
- The `password` parameter must be URL-encoded (special chars like `$` break the query string)
- No LoadMaster basic auth required on unlicensed appliance — credentials are the Progress account
- Timeout: Can take 5-15 seconds as it contacts external Progress licensing servers
...
curl -sk -X POST "https://10.0.0.69:443/accessv2" \
  ...
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"alsilicensetypes"}'
```
