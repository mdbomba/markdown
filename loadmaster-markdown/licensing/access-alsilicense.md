...
- Successfully activated "Free LoadMaster" license on firmware 7.2.63.x (KVM platform)
- `lic_type_id` used: SHA1 hash from `alsilicensetypes` response (e.g., `813448163b62a600358c3c708388e4fb51d7e369`)
- Response: `code="ok"` — `Command completed ok`
- After license activation, the appliance undergoes an internal service restart (~5-25 seconds)
- **Post-license required steps:**
  1. `access/set_initial_passwd?passwd=<new-password>` (retry with 5s gaps — appliance may not be ready immediately)
  2. `access/set?param=enableapi&value=1` (API is disabled after licensing — MUST re-enable)
- The `password` parameter must be URL-encoded for special characters
- Timeout recommendation: 90 seconds (observed completion in ~10 seconds for Free license)
...
This is an **APIv1-only** operation. The licensing flow (`readeula` → `accepteula` →
`accepteula2` → `alsilicensetypes` → `alsilicense` → `set_initial_passwd`) all use the
APIv1 `/access/` endpoint because the appliance is not yet fully initialized.

After licensing completes and the initial password is set, switch to **APIv2** (`/accessv2`
JSON POST) for all subsequent configuration. See `access/set` for APIv2 usage.
...
curl -sk -X POST "https://10.0.0.69:443/accessv2" \
  ...
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"alsilicense"}'
```
