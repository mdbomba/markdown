...
- Confirmed full licensing flow on firmware 7.2.54.x (KVM platform):
  1. `readeula` returns fresh `Magic` token (UUID format, e.g., `feb9164f-cb05-4112-aff7-602feb35cda7`)
  2. `accepteula?magic=<token>&type=free` returns `code="ok"` with a **second** `Magic` token for `accepteula2`
  3. The first magic token is **single-use** — reusing it returns `Accepteula called out of sequence`
  4. Calling without `type` parameter returns `type: String Value missing`
  5. The `type=yes` (non-enum value) returns `Unknown license model`
- Successful response includes an empty `<Eula></Eula>` element and the second `<Magic>` token
- Second magic token format is also UUID (e.g., `90e49ca5-0b01-42fb-9c15-eb02d4019842`)
...
curl -sk -X POST "https://10.0.0.69:443/accessv2" \
  ...
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"accepteula"}'
```
