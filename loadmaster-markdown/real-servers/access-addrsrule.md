...
**PS Cmdlet**: `New-AdcRealServerRule`

## Description

Associates a rule with a real server.

## Endpoint

```text
POST https://<host>:<port>/access/addrsrule?rule=<rule>[&...]
```
...
```bash
curl -sk -u "bal:PASSWORD" -X POST "https://10.0.0.69:443/access/addrsrule?rule=example-rule"
```
...
curl -sk -X POST "https://10.0.0.69:443/accessv2" \
  ...
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"addrsrule","vs":"10.0.0.50","port":"443","prot":"tcp","rs":"10.0.0.50","rsport":"443","rule":"example"}'
```
...
- `access/delrsrule` — removes a rule association from a real server
