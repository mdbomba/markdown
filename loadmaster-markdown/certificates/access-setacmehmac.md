...
curl -sk -X POST "https://<host>:<port>/accessv2" \
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"setacmehmac", "param": "value"}'
```

### APIv1 (Fallback)

```text
GET https://<host>:<port>/access/setacmehmac?hmac=<value>&acmetype=<value>
```
...
|-----------|------|----------|-------------|
| `hmac` | string | See notes | See description |
| `acmetype` | string | See notes | See description |

## Example Request (APIv1)

```bash
curl -sk -u "bal:PASSWORD" "https://10.0.0.14/access/setacmehmac?hmac=1&acmetype=2"
```
...
curl -sk -X POST "https://10.0.0.14/accessv2" \
  -d '{"apiuser": "bal", "apipass": "PASSWORD", "cmd": "setacmehmac", "hmac": "1", "acmetype": "2"}'
```
