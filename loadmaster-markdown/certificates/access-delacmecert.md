...
curl -sk -X POST "https://<host>:<port>/accessv2" \
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"delacmecert", "param": "value"}'
```

### APIv1 (Fallback)

```text
GET https://<host>:<port>/access/delacmecert?cert=<value>&acmetype=<value>
```
...
```bash
curl -sk -u "bal:PASSWORD" "https://10.0.0.14/access/delacmecert?cert=CertificateName&acmetype=1"
```
...
curl -sk -X POST "https://10.0.0.14/accessv2" \
  -d '{"apiuser": "bal", "apipass": "PASSWORD", "cmd": "delacmecert", "cert": "CertificateName", "acmetype": "1"}'
```
