...
curl -sk -X POST "https://<host>:<port>/accessv2" \
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"listacmecert", "param": "value"}'
```

### APIv1 (Fallback)

```text
GET https://<host>:<port>/access/listacmecert?acmetype=<value>
```
...
```bash
curl -sk -u "bal:PASSWORD" "https://10.0.0.14/access/listacmecert?acmetype=1"
```
...
curl -sk -X POST "https://10.0.0.14/accessv2" \
  -d '{"apiuser": "bal", "apipass": "PASSWORD", "cmd": "listacmecert", "acmetype": "1"}'
```
