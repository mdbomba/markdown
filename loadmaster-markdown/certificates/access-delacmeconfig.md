...
curl -sk -X POST "https://<host>:<port>/accessv2" \
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"delacmeconfig", "param": "value"}'
```

### APIv1 (Fallback)

```text
GET https://<host>:<port>/access/delacmeconfig?acmetype=<value>
```
...
```bash
curl -sk -u "bal:PASSWORD" "https://10.0.0.14/access/delacmeconfig?acmetype=1"
```
...
curl -sk -X POST "https://10.0.0.14/accessv2" \
  -d '{"apiuser": "bal", "apipass": "PASSWORD", "cmd": "delacmeconfig", "acmetype": "1"}'
```
