# access/setlmingresswatchtimeout
...
curl -sk -X POST "https://<host>:<port>/accessv2" \
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"setlmingresswatchtimeout", "param": "value"}'
```

### APIv1 (Fallback)

```text
GET https://<host>:<port>/access/setlmingresswatchtimeout?watchtimeout=<value>
```
...
|-----------|------|----------|-------------|
| `watchtimeout` | string | See notes | See description |

## Example Request (APIv1)

```bash
curl -sk -u "bal:PASSWORD" "https://10.0.0.14/access/setlmingresswatchtimeout?watchtimeout=30"
```
...
curl -sk -X POST "https://10.0.0.14/accessv2" \
  -d '{"apiuser": "bal", "apipass": "PASSWORD", "cmd": "setlmingresswatchtimeout", "watchtimeout": "30"}'
```
