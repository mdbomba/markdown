...
curl -sk -X POST "https://<host>:<port>/accessv2" \
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"dellmingressk8sconf"}'
```

### APIv1 (Fallback)

```text
GET https://<host>:<port>/access/dellmingressk8sconf
```
...
```bash
curl -sk -u "bal:PASSWORD" "https://10.0.0.14/access/dellmingressk8sconf"
```
