...
curl -sk -X POST "https://<host>:<port>/accessv2" \
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"showlmingressk8sconf"}'
```

### APIv1 (Fallback)

```text
GET https://<host>:<port>/access/showlmingressk8sconf
```
...
```bash
curl -sk -u "bal:PASSWORD" "https://10.0.0.14/access/showlmingressk8sconf"
```
