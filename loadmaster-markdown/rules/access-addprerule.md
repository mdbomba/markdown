...
```text
POST https://<host>:<port>/access/addprerule?vs=<vs>&rule=<rule>&port=<port>[&...]
```
...
```bash
curl -sk -u "bal:PASSWORD" -X POST "https://10.0.0.69:443/access/addprerule?vs=192.0.2.10&rule=example-rule&port=443&prot=tcp"
```
...
<Response stat="200" code="ok">
  <Success>
    <Data>
      <!-- addprerule operation completed -->
    </Data>
  </Success>
</Response>
```
...
curl -sk -X POST "https://10.0.0.69:443/accessv2" \
  ...
  -d '{"apiuser":"bal","apipass":"PASSWORD","cmd":"addprerule","vs":"10.0.0.50","rule":"example","port":"443","prot":"tcp"}'
```
...
- `access/delprerule` — deletes prerule
